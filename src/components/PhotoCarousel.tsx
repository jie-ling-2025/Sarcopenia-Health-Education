...
// @ts-ignore
import sarcfHero from '../assets/hero/sarc-f.png';
// @ts-ignore
import proteinHero from '../assets/hero/protein.png';
// @ts-ignore
import exerciseHero from '../assets/hero/exercise.png';
// @ts-ignore
import storiesHero from '../assets/hero/stories.png';
// @ts-ignore
import healthGuideHero from '../assets/hero/health-guide.png';

const slides = [
  { image: overviewHero, eyebrow: '銀髮健康基礎', title: '認識肌少症', description: '了解肌少症的常見警訊、影響與預防三大支柱，建立正確的健康觀念。', href: '#overview', action: '開始認識' },
  { image: sarcfHero, eyebrow: 'SARC-F 自我篩檢', title: '快速檢測', description: '透過五個簡單問題，初步了解肌力、行走、起立、爬樓梯與跌倒情形。', href: '#sarc-f', action: '開始檢測' },
  { image: proteinHero, eyebrow: '飲食與營養規劃', title: '蛋白質助手', description: '依體重與活動狀況試算每日蛋白質目標，再用常見食材安排日常配餐。', href: '#protein', action: '開始試算' },
  { image: exerciseHero, eyebrow: '安全居家練習', title: '居家運動', description: '跟著四套阻力運動的步驟、計時器與組數紀錄，循序漸進培養肌力。', href: '#exercise', action: '查看運動' },
  { image: storiesHero, eyebrow: '銀髮肌力勇氣劇場', title: '勇氣故事', description: '從長者開始訓練的故事，看見持續記錄、逐步改變與自主生活的力量。', href: '#stories', action: '閱讀故事' },
  { image: healthGuideHero, eyebrow: '日常生活核心指引', title: '健康知識', description: '整理維生素 D、水分、運動強度與休息安排等日常健康衛教重點。', href: '#health-guide', action: '閱讀指南' },
];
    ...
    >
      ...
      >
        {slides.map((slide, index) => (
          <article key={slide.title} className="relative min-h-[430px] w-full shrink-0 md:min-h-[560px]" aria-hidden={index !== current}>
            <img
              src={slide.image}
              alt={`${slide.title}主視覺`}
            ...
            <div className="relative z-10 mx-auto flex min-h-[430px] max-w-6xl flex-col justify-end px-8 py-14 text-white md:min-h-[560px] md:px-16 md:py-20">
              <p className="mb-3 text-sm font-black tracking-[0.2em] text-amber-300">{slide.eyebrow}</p>
              <h2 className="text-4xl font-black leading-tight drop-shadow-md md:text-6xl">{slide.title}</h2>
              <p className="mt-4 max-w-2xl text-base leading-relaxed text-white drop-shadow md:text-xl">{slide.description}</p>
